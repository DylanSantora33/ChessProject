			...
			for (int r = 0; r < NUM_ROWS; r++)
				for (int c = 0; c < NUM_COLS; c++) {
					if (event.getSource() == buttonArray[r][c]) {
						...
						if (!myBoard.getStage())
						{
							if (myBoard.getPiece(new Location(r,c)) != null && ((myBoard.getTurn() == true && myBoard.getPiece(new Location(r,c)).getMyColor() == 1) || (myBoard.getTurn() == false && myBoard.getPiece(new Location(r,c)).getMyColor() == -1))) {
